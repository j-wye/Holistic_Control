...
xy_data=np.ones((environment_dim, 2))*10
dist_goal=10
...
class td3(object):
    ...
    def train(
        ...
        for it in range(iterations):
            ...
            pp=torch.Tensor(batch_pp).to(device)
            ...
            target_Q3, target_Q4 = self.critic_target(next_state, pp)

            # Select the minimal Q value from the 2 calculated values
            target_Q1 = torch.min(target_Q1, target_Q2)
            target_Q2 = torch.min(target_Q3, target_Q4)
...
class GazeboEnv(Node):
    ...
    @staticmethod #modify reward function
    ...
    def velodyne_callback(self, v):
        ...
        for i in range(len(data)):
            if data[i][2] > -0.2:
                ...
                # 각 point의 beta 값에 따라 theta 결정
                for j, gap in enumerate(self.gaps):
                    if gap[0] <= beta < gap[1]:
                        theta = (gap[0] + gap[1]) / 2
                        x = int(dist * np.cos(theta))
                        y = int(dist * np.sin(theta))
                        xy_data[j]= [x, y]
...
def attractive_force():
    ...
    att_x=KP*x/dist_goal
    att_y=KP*y/dist_goal
...
def repulsive_force():
    ...
    # 장애물 위치 (xy_data)와 로봇 위치 (last_odom) 간 거리 계산
    dist_x = xy_data[:, 0] - last_odom.pose.pose.position.x
    dist_y = xy_data[:, 1] - last_odom.pose.pose.position.y
    
    # 유클리드 거리 계산
    obj_dist = np.sqrt(dist_x**2 + dist_y**2)
    
    # 장애물 중에서 obs_limit 내에 있는 것만 선택
    valid_indices = obj_dist < obs_limit
    
    # 유효한 거리, x, y 값 필터링
    valid_obj_dist = obj_dist[valid_indices]
    valid_dist_x = dist_x[valid_indices]
    valid_dist_y = dist_y[valid_indices]
    
    # 유효한 거리들에 대해 반발력 계산
    rep_x = np.sum(ETA * (1 / valid_obj_dist - 1 / obs_limit) * (1 / (valid_obj_dist**2)) * (valid_dist_x / valid_obj_dist))
    rep_y = np.sum(ETA * (1 / valid_obj_dist - 1 / obs_limit) * (1 / (valid_obj_dist**2)) * (valid_dist_y / valid_obj_dist))
...
def artificial_potential_field():
    # 기존 코드
    att_x, att_y = attractive_force()
    rep_x, rep_y = repulsive_force()
    mov_x, mov_y = att_x + rep_x, att_y + rep_y

    # 방향 벡터의 크기 계산
    mov_magnitude = math.sqrt(mov_x**2 + mov_y**2)

    # mov_x, mov_y의 방향 벡터
    mov_dir_x = mov_x / mov_magnitude
    mov_dir_y = mov_y / mov_magnitude

    # last_odom.pose.pose.orientation.z를 라디안 값으로 변환
    orientation_z_rad = last_odom.pose.pose.orientation.z

    # 두 벡터 사이의 각도 계산 (라디안)
    angle_rad = math.atan2(mov_dir_y, mov_dir_x) - orientation_z_rad

    # 각도를 도로 변환
    angle_deg = math.degrees(angle_rad)

    # 기존 코드 계속
    linear = math.sqrt((mov_x - last_odom.pose.pose.position.x)**2 + (mov_y - last_odom.pose.pose.position.y)**2) / TIME_DELTA
    angular = angle_deg / TIME_DELTA
    ...
    try:
        while rclpy.ok():
            if timestep < max_timesteps:
                ...
                action = network.get_action(np.array(state))
                pp=artificial_potential_field()
                action = (action + np.random.normal(0, expl_noise, size=action_dim)).clip(
                     -max_action, max_action
                )
                pp=(pp+np.random.normal(0,expl_noise,size=action_dim)).clip(-max_action,max_action)
                ...
                replay_buffer.add(state, action, pp, reward, done_bool, next_state)
