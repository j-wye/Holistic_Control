...
pkg_path = get_package_share_directory("td3_train")
...
class IsaacEnv(Node):
    ...
    def lidar_cb(self, msg):
        ...
        if length > self.lidar_dim:
            ...
            flattened_points = flattened_points[length - self.lidar_dim:]   
        if length < self.lidar_dim:
            for _ in range(self.lidar_dim - length):
                front_points.append(0)
                flattened_points.append([0, 0, 0])
        
        self.flattened_points = flattened_points
    ...
    def step(self, action, time_steps, max_episode_steps):
        ...
        reward, self.done = self.reward_function(distance_to_the_goal, time_steps, max_episode_steps)

        self.prev_dist = distance_to_the_goal

        return self.next_state, reward, self.done
    
    def reward_function(self, distance_to_the_goal, time_steps, max_episode_steps):
        # reward = 6 / (0.1 + distance_to_the_goal) ** 2.5 - 1
        # Static Sigma
        # 3*sigma = start to goal distance 
        static_sigma = self.start_to_goal_dist / 3
        reward = 1000 * (np.exp(-0.5 * (distance_to_the_goal / static_sigma) ** 2) - 0.5)
        # dynamic_sigma
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_steps', type=int, default=5001, metavar='N', help='maximum number of steps (default: 5000)')
                ...
                while not done:
                    ...
                    if len(memory) > args.batch_size:
                        ...
                        for i in range(args.updates_per_step):
                            ...
                            writer.add_scalar('av_critic_loss', av_critic_loss,updates)
                            writer.add_scalar('av_Q', av_Q,updates)
                            writer.add_scalar('max_Q', max_Q, updates)
